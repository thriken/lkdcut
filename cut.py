                ...
                if match:
                    ...
                    try:
                        ...
                        if processing_pieces and p_num.startswith('4'):
                            ...
                            # 尝试解析数据 - 优先使用旧格式逻辑
                            if len(parts) >= 22:  # 旧格式，有22个字段
                                piece_data = common_data.copy()
                                # 处理客户名称 - 新格式可能包含数字前缀
                                customer_name = parts[6]
                                # 去除开头的数字
                                import re
                                customer_name = re.sub(r'^\d+', '', customer_name)
                                ...
                                piece_data.update({
                                    'cut_width': float(parts[0]),
                                    'cut_height': float(parts[1]),
                                    'cut_x': float(parts[2]),
                                    'cut_y': float(parts[3]),
                                    'order_width': float(parts[4]),
                                    'order_height': float(parts[5]),
                                ...
                                try:
                                    ...
                                    # 从parts[4]和parts[5]解析订单尺寸
                                    try:
                                        order_width = float(parts[4])
                                    except ValueError:
                                        order_width = cut_width
                                    try:
                                        order_height = float(parts[5])
                                    except ValueError:
                                        order_height = cut_height
                                except (ValueError, IndexError) as e:
                                    print(f"基本字段解析失败: {parts[:6]}, 错误: {e}")
                                    continue

                                # 提取客户名称 - 去除开头的数字
                                customer_name = parts[6] if len(parts) > 6 else '未知客户'
                                import re
                                customer_name = re.sub(r'^\d+', '', customer_name)
                                ...
                                piece_data.update({
                                    ...
                                    'dm_code': dm_code,
                                    'order_size': f'{order_width}x{order_height}',
                            ...
                            if p_num == '3000':
                                common_data['raw_width'] = float(value)
                            elif p_num == '3001':
                                common_data['raw_height'] = float(value)
                            elif p_num == '3007':
                                ...
                                common_data['thickness'] = float(value)
        ...
        try:
            ...
            for current_count, (file_path, file, directory, existing_md5) in enumerate(need_process_files, 1):
                ...
                try:
                    ...
                    # 阶段2.3: G代码解析
                    if current_count % 20 == 0:  # 每处理20个文件提示一次
                        print(f"正在解析第 {current_count}/{len(need_process_files)} 个文件: {file}")
        ...
        try:
            ...
            for row in selected_rows:
                ...
                try:
                    ...
                    cut_width = float(size_parts[0].strip())
                    cut_height = cut_width  # 如果是正方形，高度等于宽度
                    if len(size_parts) > 1:
                        try:
                            cut_height = float(size_parts[1].strip())
                        except ValueError:
